...
for nv in range(0,len(draw),1):#,len(f),1):
    ...
    cfp.setvars(file='month_'+filt+g1.long_name+'.png')
    cfp.gopen(figsize=[20, 20],rows=4,columns=3,wspace=0.1,hspace=0.01,bottom=0.5)
    ...
    for nm in range(0,len(titls),1):#,len(f),1):
        ...
        cfp.con(g,fill=True,lines=False,colorbar=None,title=titls[nm])
