...
figdir = "/home/users/qd201969/uor_track/fig/behv3_match_%dh_%s"%(time,suffix)
fileout="/home/users/qd201969/uor_track/mdata/behv3_match_%dh_%s.nc"%(time,suffix)
calcbehv =1 
drawannual = 0
drawbox = 1
...
if calcbehv == 1:
    ...
    for nl in range(0,len(lev1),1):
        ...
        if not os.path.isfile(filname+"_"+behv[-1]) :
    ...
    for nr in range(1,len(behv),1):
        #ret=subprocess.Popen("sh /home/users/qd201969/uor_track/match_3lev.sh %s _%s_%s"
        #    %(prefix,suffix,behv[nr]),shell=True)
        #ret.wait()
        ff = open("%smatch_%s_%s/number"%(path,suffix,behv[nr]),'r')
        lines = ff.readlines()
        for nl in range(0,len(lines),1):
            term = lines[nl].strip().split(" ")
            var[0,nl,nr]=int(term[-1].strip())
