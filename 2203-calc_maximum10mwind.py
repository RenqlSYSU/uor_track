...
def max10mwind_threshold():
    ...
    var = ds['var1'].sel(time=ds.time.dt.year.isin(1980)
            ,lon=ilon,lat=ilat)
    ...
    for dtime in var.time:
        ...
        indx3=np.arange(indx2[0]-lagh,indx2[0]+lagh+1)
        for d in indx2[1:]:
            indx3=np.concatenate((indx3,np.arange(d-lagh,d+lagh+1)))
        term = ds['var1'][indx3].sel(lon=ilon,lat=ilat).data 
