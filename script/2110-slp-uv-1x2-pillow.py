...
stime  = ['2021','09','24'] # year, month, date, hour
ftime  = pd.date_range(start='2021-09-25 00',end='2021-09-25 12', freq='1H' \
        ,closed=None).strftime("%Y-%m-%d_%H:00").to_list()
ftime0 = pd.date_range(start='2021-09-25 01',end='2021-09-25 13', freq='1H' \
        ,closed=None).strftime("%Y-%m-%d_%H:00").to_list()
anim = 1 # 1 = make animation
...
path = ['/home/lzhenn/cooperate/data/Njord/',
...
for t in range(len(ftime)):
    figtle = 'S'+'-'.join(stime)+' F'+ftime[t]+' '+drawvar
    print(figtle)
    figdir = '/home/lzhenn/cooperate/fig/S_'+varname[0]+"_"+''.join(stime)+'.F'+ftime[t].strip("_:0")+'.png'
...
#====================================================================
if anim == 1:
    fn_stream = subprocess.check_output("ls -rt /home/lzhenn/cooperate/fig/S_"+\
    ...
    subprocess.run("rm -f /home/lzhenn/cooperate/fig/S_"+varname[0]+"*.png",shell=True)
