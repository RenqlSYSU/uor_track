...
figtle = ''.join(ftime)+' '+drawvar

domin_select = ['d01','d02'] #njord, path
lat_sp = 10.0  #1.0 #
lon_sp =  5.0 #1.0 #
figdir = '/home/lzhenn/cooperate/fig/case_study-'+''.join(ftime)+'.'+varname[0]+'.jpg'
...
#cnlevels = np.arange(1003,1006.3,0.2) #slp, hPa, d03
...
for i in range(len(case)):
    ...
    filedir = path+case[i]+'/wrfout_'+domin+'_'+'-'.join(ftime[0:3])+'_'+ftime[3]+':00:00'
    ...
    if varname[0] == 'RAINNC':
        ...
        filedir0 = path+case[i]+'/wrfout_'+domin+'_'+'-'.join(ftime0[0:3])+'_'+ftime0[3]+':00:00'
    ...
    axe = plt.subplot(1,3,i+1,projection=cart_proj)    #创建子图
    ...
    axe.set_title(case[i],loc='left',fontsize=SMFONT) 

#fig.subplots_adjust(bottom=0.1,wspace=0.2,hspace=0.01)
#position = fig.add_axes([0.15, 0.08, 0.7, 0.015]) #left, bottom, width, height
fig.subplots_adjust(bottom=0.5,wspace=0.2,hspace=0.01)
position = fig.add_axes([0.2, 0.48, 0.6, 0.015]) #left, bottom, width, height
